...
const Rent = ({ rents }: RentProps) => {
    ...
    const thisMonthBills = rentalBills.filter((bill) => {
        ...
        return billDate.getFullYear() === targetMonth.getFullYear() && billDate.getMonth() === targetMonth.getMonth();
    });

    const totalMonthlyRent = thisMonthBills.reduce((sum, bill) => sum + bill.rent_amount, 0);

    const totalMonthlyPaid = thisMonthBills.reduce((sum, bill) => sum + Math.min(bill.amount_paid, bill.rent_amount), 0);

    const totalOutstanding = Math.max(0, totalMonthlyRent - totalMonthlyPaid);

    const collectionRate = totalMonthlyRent ? Math.min(100, (totalMonthlyPaid / totalMonthlyRent) * 100) : 0;
        ...
        <LandlordLayout>
            <div className="space-y-6">
                <div className="container mx-auto px-4">
                    ...
                    {/* Rental Bills Table */}
                    <div className="mt-8">
                        <RentCollectionDataTable columns={rentCollectionColumns} data={rents} />
                    </div>
                </div>
            </div>
        </LandlordLayout>
